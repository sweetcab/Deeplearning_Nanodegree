...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob):
    ...
    drop=tf.contrib.rnn.DropoutWrapper(cell,input_keep_prob=1.0,output_keep_prob=keep_prob)
    cells =tf.contrib.rnn.MultiRNNCell([drop]*num_layers)
    _,state=tf.nn.dynamic_rnn(cells,rnn_inputs,dtype=tf.float32)
    return state
...
def decoding_layer_train(encoder_state, dec_cell, dec_embed_input, sequence_length, decoding_scope,
    ...
    outputs_train, _,_ =  tf.contrib.seq2seq.dynamic_rnn_decoder(cell=dec_cell,
                                                                         decoder_fn=dynamic_fn_train,
                                                                         inputs=dec_embed_input,
                                                                         sequence_length=sequence_length,
                                                                         scope=decoding_scope)
    output_layer=output_fn(outputs_train)
    return output_layer
...
def decoding_layer_infer(encoder_state, dec_cell, dec_embeddings, start_of_sequence_id, end_of_sequence_id,
    ...
    inference_logits,_,_= tf.contrib.seq2seq.dynamic_rnn_decoder(cell=dec_cell,
...
def seq2seq_model(input_data, target_data, keep_prob, batch_size, sequence_length, source_vocab_size, target_vocab_size,
    ...
    enc_embeded=tf.nn.embedding_lookup(enc_embeddings, input_data)
    
    enc_state=encoding_layer(enc_embeded, rnn_size, num_layers, keep_prob)
    
    preprocessed_target=process_decoding_input(target_data, target_vocab_to_int, batch_size)
    dec_embeddings = tf.Variable(tf.random_uniform([target_vocab_size, dec_embedding_size]))
    dec_embeded=tf.nn.embedding_lookup(dec_embeddings, preprocessed_target)
    
    (training_logits, inference_logits)=decoding_layer(dec_embeded, 
                                                       dec_embeddings, 
                                                       enc_state, 
                                                       target_vocab_size, 
                                                       sequence_length, 
